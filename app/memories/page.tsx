  ...
  return (
    <div className="mt-5 flex items-end gap-2">
      <textarea
        ...
        onKeyDown={(event) => {
          /*
           * 한글 입력 중 Enter를 눌렀을 때 등록되는 것을 방지합니다.
           * isComposing은 브라우저별 차이를 고려하여
           * keyCode === 229도 같이 확인합니다.
           */
          if (
            event.key === "Enter" &&
            !event.shiftKey &&
        ...
        rows={1}
        className="min-h-[44px] flex-1 resize-none rounded-xl border border-[#d4e8f2] bg-[#f7fcff] px-4 py-3 text-sm leading-6 text-[#554b47] outline-none transition placeholder:text-[#b7c8cf] focus:border-[#bcd9e6] focus:bg-white"
      />
      ...
      <button
        ...
        className="shrink-0 rounded-xl bg-[#dff2fb] px-4 py-3 text-sm font-semibold text-[#456572] transition hover:bg-[#ccebf7] disabled:cursor-not-allowed disabled:opacity-40"
      >
        {saving ? "..." : "등록"}
      </button>
    </div>
...
/* =========================================================
   댓글 영역
   중요:
   이것도 MemoriesPage 밖에 둡니다.
   그래야 부모 페이지가 다시 렌더링되어도
   댓글 입력창 컴포넌트가 재생성되지 않습니다.
========================================================= */
...
function CommentsSection({
  ...
  return (
    <div className="mt-10 border-t border-[#d4e8f2] pt-8">
      <div className="mb-4">
        <p className="text-xs font-semibold tracking-wide text-[#8b9fa9]">
          COMMENTS
        </p>

        <p className="mt-1 text-sm font-semibold text-[#554b47]">
          우리들의 댓글{" "}
          <span className="text-[#9eb0b9]">
      ...
      {memoryComments.length > 0 && (
        <div className="space-y-3">
          {memoryComments.map((comment) => (
            <div
              key={comment.id}
              className="rounded-2xl bg-[#f7fcff] px-4 py-3"
            >
              <div className="flex items-center justify-between gap-3">
                ...
                {comment.created_by === myUserId && (
                  <button
                    ...
                    className="text-[10px] text-[#b5a5a0] hover:text-red-400"
                  >
                    삭제
                  </button>
                )}
      ...
      <p className="mt-2 text-[10px] text-[#a3b4bc]">
...
export default function MemoriesPage() {
  ...
  async function deletePart() {
    ...
    const partMemories = getPartMemories(
      editingPart.id
    );
    ...
    if (partMemories.length > 0) {
      ...
      if (partPhotos.length > 0) {
        const filePaths = partPhotos.map(
  ...
  async function deleteMemory() {
    ...
    if (memoryPhotos.length > 0) {
      const filePaths = memoryPhotos.map(
        (photo) => photo.file_path
      );
